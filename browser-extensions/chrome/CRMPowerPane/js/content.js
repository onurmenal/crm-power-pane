(function () {
    ...
    if (mainBody && mainBody.length > 0) {
        ...
        if (ribbon != null && ribbon.length > 0 && !helperPaneButton) {
            ...
            helperPaneButton.setAttribute('title', 'Crm Js Fiddle | Show crm helper pane.');
            helperPaneButton.innerHTML = '<a class="navTabButtonLink" title="Show crm helper pane.">'
                                            + '<span class="navTabButtonImageContainer">'
                                                + '<img alt="Crm Js Fiddle | Show crm helper pane." src="' + chrome.extension.getURL("img/icon-24.png") + '"'
                                            + '</span>'
                                         + '</a>'
            ...
            xmlHttp.onreadystatechange = function () {
                if (xmlHttp.readyState == XMLHttpRequest.DONE) {
                    if (xmlHttp.status == 200) {
                        localStorage.setItem("crmJsFiddlePath", chrome.extension.getURL(""));
